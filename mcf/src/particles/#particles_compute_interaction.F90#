	...
	!----------------------------------------------------
        ...
        !TYPE(ppm_type_ptr_to_clist), DIMENSION(:), POINTER :: cell_list  qxb
        ...
        REAL(MK), DIMENSION(3)          :: fbpip
        REAL(MK), DIMENSION(3)          :: fbpjp
...
#if __TRACER 
        !----------------------------------------------------
        ! Diffusion of tracers in bulk
        !
        ! dp         : diffusion coefficient
        !----------------------------------------------------
        
        REAL(MK)                        :: dp
        REAL(MK)                        :: dnip, nid, njd
#endif
  	!----------------------------------------------------
...
#if __TRACER
        dp = physics_get_tracers_diffusion(this%phys, stat_info_sub)
#endif
        !----------------------------------------------------
        ! Boundary parameters :
        !
        ...
        wall_noslip    = &
             boundary_get_wall_noslip_type(d_boundary,stat_info_sub)
        num_sym        = &
             boundary_get_num_sym(d_boundary,stat_info_sub)
        num_wall_sym   = &
        ...
        IF (ASSOCIATED(this%f_bp)) THEN
           DEALLOCATE(this%f_bp,STAT=stat_info_sub)
        END IF
        ...
        IF(  symmetry .OR. &
           ...
           ALLOCATE(this%f(num_dim,this%num_part_all), &
                STAT=stat_info_sub)
           ALLOCATE(this%f_bp(num_dim,this%num_part_all), &
                STAT=stat_info_sub)
        ...
        ELSE
           ...
           ALLOCATE(this%f(num_dim,this%num_part_real), &
                STAT=stat_info_sub) 
           ALLOCATE(this%f_bp(num_dim,this%num_part_real), &
                STAT=stat_info_sub)
        ...
        !----------------------------------------------------
        ! Allocate memory tracer number evolution
        ! (in the same way as force)
        !----------------------------------------------------

        IF (ASSOCIATED(this%dn)) THEN
           DEALLOCATE(this%dn,STAT=stat_info_sub)
        END IF
        
        IF(  symmetry .OR. &
             num_wall_sym > 0 .OR. num_le > 0 ) THEN
           
           ALLOCATE(this%dn(this%num_part_all), &
                STAT=stat_info_sub)
           
        ELSE
           
           ALLOCATE(this%dn(this%num_part_real), &
                STAT=stat_info_sub)
        ...
        !----------------------------------------------------
        ! Get the cell list from an object of technique class.
        !----------------------------------------------------
        ...
        DO idom = 1, num_sub
           ...
           !icend = num_cell_dim_sub(1,idom)-2 qxb
           !jcend = num_cell_dim_sub(2,idom)-2 qxb
           icend = cell_list(idom)%nm(1)-2
           jcend = cell_list(idom)%nm(2)-2        
           
           !n1  = num_cell_dim_sub(1,idom) qxb
           n1 = cell_list(idom)%nm(1)     
           ...
           ELSE              
              ...
              !     num_cell_dim_sub(2,idom) qxb
              kcend = cell_list(idom)%nm(3)-2
              n2    = cell_list(idom)%nm(1) * &
                   cell_list(idom)%nm(2)
...
#include "pp_interaction.inc"
                             ...
                             this%f_bp(1:num_dim,ip) = &
                                  this%f_bp(1:num_dim,ip) + fbpip(1:num_dim)


#if __TRACER   
                             !----------------------------------
                             ! Add up tracer interchange from jp to ip
                             !----------------------------------
                             this%dn(ip) = this%dn(ip) + dnip
#endif
                             ...
                             IF ( symmetry .OR. &
                                ...
                                this%f_bp(1:num_dim,jp) = &
                                     this%f_bp(1:num_dim,jp) + fbpjp(1:num_dim)

#if __TRACER   
                                ! also number of tracers
                                this%dn(jp) = this%dn(jp) - dnip
#endif
