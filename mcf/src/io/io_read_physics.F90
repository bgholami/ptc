	...
	REAL(MK)                        :: k_sm
        ...
        REAL(MK),ALLOCATABLE,DIMENSION(:,:,:)   :: coll_rot_matrix
        ...
        INTEGER                                 :: theta_index
        INTEGER                                 :: omega_index  
        INTEGER                                 :: arbitrary_index
        CHARACTER(LEN=MAX_CHAR)        	        :: coll_arbitrary_file        
        !----------------------------------------------------
        ! boundary variables.
        !----------------------------------------------------
        ...
        INTEGER                         :: wall_noslip    
        CHARACTER(LEN=MAX_CHAR)         :: bcdef_inout_file       
        ...
        REAL(MK)                        :: tracers_radius
        REAL(MK)                        :: tracers_rho_ratio
        INTEGER                         :: tracers_c_factor       
        REAL(MK)                        :: tracers_dp
        REAL(MK)                        :: tracers_near_wall
        INTEGER                         :: tracers_restart_num   

        !----------------------------------------------------
        ! Tracers adhesion parameters.
        !----------------------------------------------------

        INTEGER                         :: tracers_nbond
        REAL(MK)                        :: tracers_K0  
        REAL(MK)                        :: tracers_a
        REAL(MK)                        :: tracers_b
        REAL(MK)                        :: tracers_c
	...
	!----------------------------------------------------
        ...
        iline = 0
          ...
          ELSE IF (carg == 'RHO_VAR') THEN

             !-----------------------------------------------
             ! Density variation
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) rho_var

             CALL physics_set_rho_var(phys,rho_var,stat_info_sub)


          ELSE IF (carg == 'BP_FACTOR') THEN

             !-----------------------------------------------
             ! Background pressure factor
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) bp_factor

             CALL physics_set_bp(phys, bp_factor,stat_info_sub) 


          ELSE IF (carg == 'LEN_REF') THEN

             !-----------------------------------------------
             ! Reference length
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) len_ref

             CALL physics_set_len_ref(phys,len_ref,stat_info_sub)    
          ...
          ELSE IF (carg == 'VEL_REF') THEN

             !-----------------------------------------------
             ! Reference velocity
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) vel_ref

             CALL physics_set_vel_ref(phys,vel_ref,stat_info_sub)
          ...
          ELSE IF(carg == 'NUM_COLLOID' ) THEN
             ...
             ALLOCATE(coll_mmi(num_colloid))
             ALLOCATE(coll_x(num_dim,num_colloid))
             ALLOCATE(coll_v(num_dim,num_colloid))
             ALLOCATE(coll_rot_matrix(num_dim,num_dim,num_colloid))
             ...
             coll_x(:,:)       = 0.0_MK
             coll_v(:,:)       = 0.0_MK
             coll_rot_matrix(:,:,:) = 0.0_MK
             
             DO i=1,num_dim
                coll_rot_matrix(i,i,:) = 1.0_MK
             END DO
          ...
          ELSE IF (carg == 'COLL_SHAPE' .AND. &
             ...
             ! if arbitrary shape
             IF (coll_shape(shape_index) == &
                  mcf_colloid_shape_arbitrary) THEN
          ...
          ELSE IF (carg == 'COLL_ROT_MATRIX' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0 .AND. &
               rot_index >= 0 .AND.  &
               rot_index <num_colloid ) THEN
             
             rot_index = rot_index + 1
             
             READ(cvalue,*,IOSTAT=ios, ERR=200) &
                  coll_rot_matrix(1:num_dim,1:num_dim,rot_index)
          ...
          ELSE IF (carg == 'COLL_ARBITRARY_FILE' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0 .AND. &
               arbitrary_index > 0 ) THEN
             
             coll_arbitrary_file = TRIM(ADJUSTL(cvalue))
          ...
          ELSE IF (carg == 'BCDEF_INOUT_FILE') THEN

             bcdef_inout_file = TRIM(ADJUSTL(cvalue))

#if __TRACER

             !#---------------------------------------------#
             !#---------------------------------------------#
             ! Get tracers' specific parameters.
             !#---------------------------------------------#             
             !#---------------------------------------------#
             
          ELSE IF (carg == 'TRACERS_RADIUS') THEN
             
             !-----------------------------------------------
             ! Tracers' radius.
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_radius
             
             CALL physics_set_tracers_radius(phys,tracers_radius,stat_info_sub)


          ELSE IF (carg == 'TRACERS_RHO_RATIO') THEN
             
             !-----------------------------------------------
             ! Tracers' density ratio.
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_rho_ratio
             
             CALL physics_set_tracers_rho_ratio(phys,tracers_rho_ratio,stat_info_sub)    


          ELSE IF (carg == 'TRACERS_C_FACTOR') THEN
             
             !-----------------------------------------------
             ! Tracers' c factor.
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_c_factor

             CALL physics_set_tracers_c_factor(phys,tracers_c_factor,stat_info_sub) 


         ELSE IF (carg == 'TRACERS_DP') THEN
             
             !-----------------------------------------------
             ! Tracers' diffusion coeff.
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_dp
             
             CALL physics_set_tracers_diffusion(phys,tracers_dp,stat_info_sub)         


          ELSE IF (carg == 'TRACERS_NEAR_WALL') THEN
             
             !-----------------------------------------------
             ! Tracers' near-wall distance.
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_near_wall
             
             CALL physics_set_tracers_near_wall(phys,tracers_near_wall,stat_info_sub)


          ELSE IF (carg == 'TRACERS_RESTART_NUM') THEN
             
             !-----------------------------------------------
             ! number of Tracers (in case of external read for tracers)
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_restart_num
             
             CALL physics_set_tracers_restart_num(phys,tracers_restart_num,stat_info_sub)   

          ELSE IF (carg == 'TRACERS_NBOND') THEN
             
             !-----------------------------------------------
             ! adhesion: number of bonds 
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_nbond
             
             CALL physics_set_tracers_nbond(phys,tracers_nbond,stat_info_sub)  

          ELSE IF (carg == 'TRACERS_K0') THEN
             
             !-----------------------------------------------
             ! adhesion: binding affinity parameter 
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_K0
             
             CALL physics_set_tracers_K0(phys,tracers_K0,stat_info_sub)     

          ELSE IF (carg == 'TRACERS_A') THEN
             
             !-----------------------------------------------
             ! adhesion: binding affinity parameter 
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_a
             
             CALL physics_set_tracers_A(phys,tracers_a,stat_info_sub)  
             
          ELSE IF (carg == 'TRACERS_B') THEN        
             
             !-----------------------------------------------
             ! adhesion: binding affinity parameter 
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_b
             
             CALL physics_set_tracers_B(phys,tracers_b,stat_info_sub)     

          ELSE IF (carg == 'TRACERS_C') THEN
             
             !-----------------------------------------------
             ! adhesion: binding affinity parameter 
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_c
             
             CALL physics_set_tracers_C(phys,tracers_c,stat_info_sub)          

          ELSE IF (carg == 'TRACERS_D') THEN
             
             !-----------------------------------------------
             ! adhesion: binding affinity parameter 
             !-----------------------------------------------

             READ(cvalue,*,IOSTAT=ios, ERR=200) tracers_d
             
             CALL physics_set_tracers_D(phys,tracers_d,stat_info_sub)
...
9999	CONTINUE
        ...
        IF ( stat_info == 0 ) THEN
           ...
           CALL boundary_new(d_boundary,&
                num_dim,stat_info_sub)
           CALL boundary_set_bcdef(d_boundary,&
                bcdef(1:2*num_dim),stat_info_sub)
           CALL boundary_set_shear_type(d_boundary,&
                shear_type(1:2*num_dim),stat_info_sub)
           CALL boundary_set_shear_v0(d_boundary,&
                shear_v(1:num_dim,1:2*num_dim),stat_info_sub)
           CALL boundary_set_shear_freq(d_boundary,&
                shear_freq(1:2*num_dim),stat_info_sub)
           CALL boundary_set_wall_rho_type(d_boundary, &
                wall_rho_type, stat_info_sub)
           CALL boundary_set_wall_noslip_type(d_boundary,&
                wall_noslip,stat_info_sub)
           CALL boundary_set_min_phys(d_boundary,&
                min_phys(1:num_dim),stat_info_sub)
           CALL boundary_set_max_phys(d_boundary,&
                max_phys(1:num_dim),stat_info_sub)        

           !----------------------------------------------
           ! inflow/outflow boundary patch
           !---------------------------------------------- 
           inout_BC = .FALSE.
           DO i = 1, 2*num_dim
              IF ((bcdef(i) == mcf_bcdef_inflow) .OR. &
                   (bcdef(i) == mcf_bcdef_outflow)) THEN
                 inout_BC = .TRUE.
              END IF
           END DO

           IF (inout_BC) THEN
              CALL boundary_set_inflow_outflow(d_boundary, &
                   bcdef_inout_file, stat_info_sub)
           ...
           IF ( num_colloid > 0 ) THEN
              ...
              CALL colloid_set_mmi(colloids,&
                   coll_mmi(1:num_colloid),stat_info_sub)
              ...
              CALL colloid_set_rotation_matrix(colloids,&
                   coll_rot_matrix(1:num_dim,1:num_dim,1:num_colloid),&
                   stat_info_sub)
              ...
              CALL colloid_set_bcdef(colloids,&
                   bcdef(1:2*num_dim),stat_info_sub)
              CALL colloid_set_boundary(colloids,d_boundary,&
                   stat_info_sub)
              ...
              !----------------------------------------------
              ! properties of arbitrary shape colloid
              !----------------------------------------------  
	...
	!----------------------------------------------------
        ...
        IF(ALLOCATED(coll_rot_matrix))THEN
