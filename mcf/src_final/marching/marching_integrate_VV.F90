	...
	!----------------------------------------------------
	! set velocities for inflow/outflow particles
	!----------------------------------------------------

        IF ( num_inout > 0 ) THEN
           
           CALL particles_set_inflow_outflow_velocity(this%particles, &
	...
	! Position integration r(t+dt) = 
        ...
        ! however, since we integrated v first already above
        ! as v'(t+dt) = v(t) + 0.5 * f(t) * dt,
        ! by choosing 1, here we have r(t+dt) = r(t) + v'(t)*dt.
        ! To have Transport velocity formulation, we use 2 to
        ! have  r(t+dt) = r(t) + v'(t) * dt  + 0.5*f_bp(t)*dt**2,
	!----------------------------------------------------
        
        CALL particles_integrate_position(this%particles, &
             num_part_real, dt, 2, stat_info_sub)
        ...
        !----------------------------------------------------
	! Evolve c_tracers on SPH particles
	!----------------------------------------------------
        
        CALL particles_evolve_ctracers(this%particles, &
             num_part_real,dt,stat_info_sub)
        
        IF ( stat_info_sub /= 0 ) THEN
           PRINT *, "marching_integrate_VV : ", &
                "Evolving c_tracers failed ! "
           stat_info = -1
           GOTO 9999
        END IF
	...
	!----------------------------------------------------
        ...
        IF ( num_colloid > 0 ) THEN           
           ...
           ! since we have integrated velocity already,
           ...
           CALL colloid_integrate_angle(colloids,dt, &
                1,stat_info_sub)
           ...
           CALL particles_integrate_colloid_position(this%particles,&
                dt,1,stat_info_sub)
        ...
        IF ( num_shear > 0 ) THEN
           ...
           CALL boundary_update_boundary(d_boundary, &
        ...
        !     num_part_real,stat_info_sub)   
        CALL particles_adjust_particles_special(this%particles,&
             num_part_real,stat_info_sub)
	...
	! Has to be done BEFORE building neighbor lists.
        ...
        !----------------------------------------------------
        ! symmtery inter-process communication:
        !----------------------------------------------------
	...
	! Apply external / body force for real particles.
        ...
        IF ( num_shear > 0 ) THEN
           ...
           CAll boundary_collect_colloid_interaction(d_boundary,comm, &
	...
	!----------------------------------------------------
        ...
        IF (SIZE(this%tracers%num_interface) /= 0) THEN   
           ...
           CALL tracers_integrate(this%tracers, &
                this%particles, &
                time, &
                dt, &
                stat_info_sub)   
