...
! name mcf_ctrl. 
! Once the member functions/subroutiens of object
...
!-------------------------------------------------------------
      ...
      PROGRAM mcf
...
#if __TRACER
        ! a tracer is a type of particle.
        TYPE(Particles)                 :: mcf_tracers
#endif
...
#if __TRACER
        read_tracers       = &
             control_get_read_tracers(mcf_ctrl,stat_info_sub)
#endif
        kernel_type        = &
       ...
       ! since it will be used for calculating dt.
        ...
        !----------------------------------------------------
        p0 = stateEquation_get_p0(mcf_stateEquation, stat_info_sub)
        bp = physics_get_bp(mcf_phys, stat_info_sub)
        CALL physics_set_bp(mcf_phys, p0 * bp, stat_info_sub)
        ...
        ! by importing various objects which
        ! will be used by the Tracers object.
        !----------------------------------------------------

        CALL tracers_new(mcf_tracers,mcf_ctrl,mcf_phys,&
             mcf_rhs,mcf_stateEquation,mcf_kern,mcf_tech,&
             stat_info_sub)
        ...
        CALL marching_new(mcf_marching,&
             mcf_io,mcf_particles,mcf_tracers,stat_info_sub)
#else      
        CALL marching_new(mcf_marching,&
        ...
        IF ( rank == 0 ) THEN
           ...
           IF( read_external ) THEN
              ...
              !----------------------------------------------
              ! Read tracers externally,
              !----------------------------------------------
#if __TRACER 
              IF (read_tracers) THEN
                 CALL io_read_tracers(mcf_io,rank,&
                      mcf_tracers,stat_info_sub)
              END IF
#endif
        ...
        IF ( read_external ) THEN
           ...
           IF (read_tracers) THEN 
              ...
              CALL tracers_decompose_ring(mcf_tracers,&
                   l_map_x    = .TRUE., l_map_v  = .TRUE., &
                   l_map_id   = .TRUE., &
                   stat_info  = stat_info_sub )   
        ...
        ! create interface
        CALL tracers_interface_create(mcf_tracers, &
             stat_info = stat_info_sub)   

        ! if external read for tracers, then read from file
        ! (only rank 0)
        IF ((rank == 0) .AND. (read_tracers)) THEN
           
           CALL io_read_deposited(mcf_io, mcf_tracers, stat_info_sub)
        ...
        !----------------------------------------------------
        ! In case of arbitrary colloid, 
        ! we get rid of extra particles here.
        !----------------------------------------------------
        
        IF( .NOT. read_external ) THEN


           CALL particles_init_partial_inter_adjust(mcf_particles,&
                stat_info_sub) 
...
#if __TRACER
        CALL tracers_finalize(mcf_tracers,stat_info_sub)
#endif
