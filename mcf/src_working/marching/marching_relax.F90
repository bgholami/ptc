      SUBROUTINE marching_relax(this,stat_info)
        ...
        TYPE(StateEquation), POINTER    :: d_stateEquation
	...
	! Debug variables.
        ...
        CALL particles_get_stateEquation(this%particles,&
             d_stateEquation,stat_info_sub)
        CALL stateEquation_new(d_stateEquation,&
             stateEquation_type,&
             c_relax,rho,rho_ref,gamma,stat_info_sub)
        
        !----------------------------------------------------
        ! 2) Set dt and kt in right hand side object.
        ...
        CALL rhs_set_Brownian(d_rhs,.TRUE.,stat_info_sub)
        CALL rhs_set_dt(d_rhs,dt_relax,stat_info_sub)
        CALL rhs_set_kt(d_rhs,kt_relax,stat_info_sub)
        ...
        CALL boundary_get_shear_v0(d_boundary, &
        ...
        IF (num_wall_solid > 0) THEN
           ...
           wall_rho_type =  &
                boundary_get_wall_rho_type(d_boundary,stat_info_sub)
	...
	! Has to be done BEFORE building neighbor list.
        ...
        CALL rhs_set_Brownian(d_rhs,.FALSE.,stat_info_sub)
        CALL rhs_set_dt(d_rhs,dt,stat_info_sub)
        CALL rhs_set_kt(d_rhs,0.0_MK,stat_info_sub)
        ...
        CALL rhs_set_Brownian(d_rhs,Brownian,stat_info_sub)
        CALL rhs_set_dt(d_rhs,dt,stat_info_sub)
        CALL rhs_set_kt(d_rhs,kt,stat_info_sub)
