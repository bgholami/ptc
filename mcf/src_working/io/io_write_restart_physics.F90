	...
	! Arguments.
        ...
        NULLIFY(shear_type)
        NULLIFY(shear_freq)
        CALL physics_get_boundary(d_physics,d_boundary,stat_info_sub)
        CALL boundary_get_shear_type(d_boundary,shear_type,stat_info_sub)
        CALL boundary_get_shear_v(d_boundary,shear_v,stat_info_sub)
        CALL boundary_get_shear_freq(d_boundary,shear_freq,stat_info_sub)
        wall_rho_type = &
        ...
        IF( num_species > 1 .AND. num_colloid > 0 ) THEN 
           ...
           DO i = 1, num_colloid
              ...
              WRITE(cbuf, '(A,E16.8)') 'coll_mmi = ', coll_mmi(i)
              WRITE(UNIT=this%restart_physics_unit,FMT='(A)',IOSTAT=stat_info_sub)  TRIM(cbuf)
              WRITE(UNIT=this%restart_physics_unit,FMT='(A)',IOSTAT=stat_info_sub) ' '
              ...
              IF(num_dim == 2) THEN
                 WRITE(cbuf, '(4(A,E16.8))') &
                      'rot_matrix = ', coll_rot_matrix(1,1,i),&
                      ',', coll_rot_matrix(2,1,i), ',', coll_rot_matrix(1,2,i),&
                      ',', coll_rot_matrix(2,2,i)
              ELSE IF (num_dim == 3 ) THEN
                 WRITE(cbuf, '(9(A,E16.8))') &
                      'rot_matrix = ',coll_rot_matrix(1,1,i),&
                      ',', coll_rot_matrix(2,1,i), ',', coll_rot_matrix(3,1,i),&
                      ( (',', coll_rot_matrix(k,j,i),k=1,3),j=2,3 )
              END IF
