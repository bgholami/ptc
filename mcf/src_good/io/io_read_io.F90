	...
	!----------------------------------------------------
        ...
        DO
           ...
           ELSE  IF (carg == 'OUTPUT_TRACERS_FILE') THEN
              
              this%output_tracers_file = TRIM(ADJUSTL(cvalue))
              
           ELSE IF (carg == 'OUTPUT_TRACERS_FMT') THEN
              
              this%output_tracers_fmt = TRIM(ADJUSTL(cvalue))
              
           ELSE IF (carg == 'OUTPUT_TRACERS_FREQ_STEP') THEN
              
              READ(cvalue,*,IOSTAT=ios, ERR=200) &
                   this%output_tracers_freq_step
              
           ELSE IF (carg == 'OUTPUT_TRACERS_FREQ_TIME') THEN
              
              READ(cvalue,*,IOSTAT=ios, ERR=200) &
                   this%output_tracers_freq_time  

           ELSE  IF (carg == 'READ_TRACERS_FILE') THEN
              
              this%read_tracers_file = TRIM(ADJUSTL(cvalue))  

           ELSE  IF (carg == 'READ_DEPOSITED_FILE') THEN
              
              this%read_deposited_file = TRIM(ADJUSTL(cvalue))
              
           ELSE IF (carg == 'READ_TRACERS_FMT') THEN
              
              this%read_tracers_fmt = TRIM(ADJUSTL(cvalue))          
