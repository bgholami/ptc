...
!-------------------------------------------------
      ...
      SUBROUTINE colloid_get_mmi(this,d_mmi, stat_info)
        ...
        ALLOCATE(d_mmi(this%num_colloid))
        
        d_mmi(:) = this%mmi(1:this%num_colloid)
      ...
      SUBROUTINE colloid_get_rotation_matrix(this,d_matrix,stat_info)
        ...
        ALLOCATE(d_matrix(this%num_dim,this%num_dim,this%num_colloid))
      ...
      SUBROUTINE colloid_get_phi(this,d_phi,stat_info)
        !---------------------------------------
        ! Return roated angle.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)         :: this
        REAL(MK), DIMENSION(:,:), POINTER :: d_phi
        INTEGER, INTENT(OUT)              :: stat_info
        
        stat_info = 0
        
        IF(ASSOCIATED(d_phi))THEN
           DEALLOCATE(d_phi)
        END IF
        
        ALLOCATE(d_phi(3,this%num_colloid))
        
        d_phi(:,:) = this%phi(:,1:this%num_colloid)
        
        RETURN       
        
      END SUBROUTINE colloid_get_phi
...
9999    CONTINUE
 ...
 INTEGER FUNCTION colloid_get_arbitrary_num(this, stat_info)
        !---------------------------------------
        !   Return the num of arbitrary shape colloids.
        !---------------------------------------
        ...
        stat_info = 0
        colloid_get_arbitrary_num = this%coll_arbitrary_num 
        
        RETURN
        
      END FUNCTION colloid_get_arbitrary_num
      

      SUBROUTINE colloid_get_v_num(this, d_v_num, stat_info)
        !---------------------------------------
        ! Return the number of vertices of each arbitrary colloid.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)       :: this
        INTEGER, DIMENSION(:), POINTER  :: d_v_num        
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        IF(ASSOCIATED(d_v_num))THEN
           DEALLOCATE(d_v_num)
        END IF

        ALLOCATE(d_v_num(this%coll_arbitrary_num))
        
        d_v_num(:) = this%coll_v_num(1:this%coll_arbitrary_num)
        
        RETURN       
        
      END SUBROUTINE colloid_get_v_num    


      SUBROUTINE colloid_get_f_num(this, d_f_num, stat_info)
        !---------------------------------------
        ! Return the number of facets of each arbitrary colloid.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)       :: this
        INTEGER, DIMENSION(:), POINTER  :: d_f_num        
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        IF(ASSOCIATED(d_f_num))THEN
           DEALLOCATE(d_f_num)
        END IF

        ALLOCATE(d_f_num(this%coll_arbitrary_num))
        
        d_f_num(:) = this%coll_f_num(1:this%coll_arbitrary_num)
        
        RETURN       
        
      END SUBROUTINE colloid_get_f_num


      SUBROUTINE colloid_get_coll_v(this, d_coll_v, stat_info)
        !---------------------------------------
        ! Return the coll_v.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)               :: this
        REAL(MK), DIMENSION(:,:,:), POINTER     :: d_coll_v
        INTEGER, INTENT(OUT)                    :: stat_info
        
        
        stat_info = 0
        
        IF(ASSOCIATED(d_coll_v)) THEN           
           DEALLOCATE(d_coll_v)
        END IF
        
        ALLOCATE(d_coll_v(this%coll_arbitrary_num, this%num_dim, &
             MAXVAL(this%coll_v_num)))
        
        d_coll_v(:,:,:) = this%coll_v( &
             1:this%coll_arbitrary_num, 1:this%num_dim, 1:MAXVAL(this%coll_v_num))
        
        RETURN       
        
      END SUBROUTINE colloid_get_coll_v    


      SUBROUTINE colloid_get_coll_v_flist(this, d_coll_v_flist, stat_info)
        !---------------------------------------
        ! Return the coll_v_flist.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)               :: this
        INTEGER , DIMENSION(:,:,:), POINTER     :: d_coll_v_flist
        INTEGER, INTENT(OUT)                    :: stat_info
        ...
        IF(ASSOCIATED(d_coll_v_flist)) THEN           
           DEALLOCATE(d_coll_v_flist)
        END IF
        
        max_degree = SIZE(this%coll_v_flist, 2)

        ALLOCATE(d_coll_v_flist(this%coll_arbitrary_num, max_degree, &
             MAXVAL(this%coll_v_num)))
        
        d_coll_v_flist(:,:,:) = this%coll_v_flist( &
             1:this%coll_arbitrary_num, 1:max_degree, 1:MAXVAL(this%coll_v_num))
        
        RETURN       
        
      END SUBROUTINE colloid_get_coll_v_flist      


      SUBROUTINE colloid_get_coll_f_vlist(this, d_coll_f_vlist, stat_info)
        !---------------------------------------
        ! Return the coll_f_vlist.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)               :: this
        INTEGER , DIMENSION(:,:,:), POINTER     :: d_coll_f_vlist
        INTEGER, INTENT(OUT)                    :: stat_info
        ...
        IF(ASSOCIATED(d_coll_f_vlist)) THEN           
           DEALLOCATE(d_coll_f_vlist)
        END IF
        
        vperf = SIZE(this%coll_f_vlist, 2)

        ALLOCATE(d_coll_f_vlist(this%coll_arbitrary_num, vperf, &
             MAXVAL(this%coll_f_num)))
        
        d_coll_f_vlist(:,:,:) = this%coll_f_vlist( &
             1:this%coll_arbitrary_num, 1:vperf, 1:MAXVAL(this%coll_f_num))
        
        RETURN       
        
      END SUBROUTINE colloid_get_coll_f_vlist



      SUBROUTINE colloid_get_coll_n(this, d_coll_n, stat_info)
        !---------------------------------------
        ! Return the coll_n.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)               :: this
        REAL(MK), DIMENSION(:,:,:), POINTER     :: d_coll_n
        INTEGER, INTENT(OUT)                    :: stat_info
        
        
        stat_info = 0
        
        IF(ASSOCIATED(d_coll_n)) THEN           
           DEALLOCATE(d_coll_n)
        END IF
        
        ALLOCATE(d_coll_n(this%coll_arbitrary_num, this%num_dim, &
             MAXVAL(this%coll_f_num)))
        
        d_coll_n(:,:,:) = this%coll_n( &
             1:this%coll_arbitrary_num, 1:this%num_dim, 1:MAXVAL(this%coll_f_num))
        
        RETURN       
        
      END SUBROUTINE colloid_get_coll_n
