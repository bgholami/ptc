      SUBROUTINE physics_adjust_parameters(this,stat_info)
        ...
        DO i = 1, num_dim
           ...
           ! do almost the same in case of inflow/outflow
           IF ( (bcdef(2*i-1) == mcf_bcdef_inflow) .OR. &
                (bcdef(2*i-1) == mcf_bcdef_outflow)) THEN
              
              IOghost_layer = mcf_IOghost_layer_coeff * cut_off
              
              num_part_IOghost(2*i-1) = &
                   CEILING(IOghost_layer / dx(i))
              
              min_phys_t(i) = min_phys_t(i) - &
                   num_part_IOghost(2*i-1) * dx(i)
          ...
          IF ( (bcdef(2*i) == mcf_bcdef_inflow) .OR. &
                (bcdef(2*i) == mcf_bcdef_outflow)) THEN
           
              IOghost_layer = mcf_IOghost_layer_coeff * cut_off
              
              num_part_IOghost(2*i) = &
                   CEILING(IOghost_layer / dx(i))
              
              max_phys_t(i) = max_phys_t(i) + &
                   num_part_IOghost(2*i) * dx(i)
